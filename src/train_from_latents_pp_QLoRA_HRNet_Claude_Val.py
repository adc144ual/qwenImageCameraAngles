...
def validate(
    ...
    """
    NUEVO V2: Calcula validation loss.
    
    Args:
        model_split: Modelo distribuido
        val_dataloader: DataLoader de validación
        loss_fn: Función de loss (solo en Rank 1)
        diff_scheduler: Scheduler de difusión
        device: Device actual
        rank: Rank del proceso
        dtype: Dtype de trabajo
    
    Returns:
        val_loss promedio (solo válido en Rank 0 después de broadcast)
    """
    model_split.eval()
    ...
    for batch in val_dataloader:
        ...
        if rank == 0:
            ...
            intermediate = model_split(*inputs)
            
            # Comunicar a Rank 1
            meta = [{"shape": list(t.shape), "dtype": str(t.dtype).split('.')[-1]} 
            ...
            # No necesitamos recibir nada de vuelta en validación
            
        elif rank == 1:
            # Recibir desde Rank 0
            container = [None]
            ...
            for info in meta:
                ...
                tensor = torch.empty(info["shape"], dtype=dtype_t, device=device)
    ...
    if rank == 1 and num_batches > 0:
        avg_loss = total_loss / num_batches
    else:
        avg_loss = 0.0
    
    # Sincronizar val_loss entre ranks
...
# ---------------------------------------------------------------------------
# Model wrapper con pipeline split (sin cambios)
# ---------------------------------------------------------------------------
...
class QwenSplitWrapper(nn.Module):
    ...
    def _compute_img_shapes(self, hidden_states: torch.Tensor, batch_size: int) -> List:
        ...
        if grid_sq * grid_sq != half:
            raise ValueError(f"_compute_img_shapes: seq_len={seq_len} inválido")
    ...
    def forward(self, *inputs):
        if self.rank == 0:
            ...
            return (hidden_states, encoder_hidden_states, mask_float, temb, 
                    r0.real.contiguous(), r0.imag.contiguous(), 
                    r1.real.contiguous(), r1.imag.contiguous())
...
# ---------------------------------------------------------------------------
# Inferencia callback (sin cambios)
# ---------------------------------------------------------------------------
...
def main():
    ...
    if os.path.exists(checkpoint_path):
        logger.info(f"Rank {rank}: Checkpoint detectado en {checkpoint_path}, cargando pesos LoRA...")
        try:
            ...
            missing, unexpected = model_split.load_state_dict(lora_state, strict=False)
            loaded_keys = len(lora_state) - len(unexpected)
            logger.info(
                f"Rank {rank}: {loaded_keys}/{len(lora_state)} pesos LoRA cargados "
                f"({len(missing)} faltantes, {len(unexpected)} inesperados)"
            )
        except Exception as e:
            logger.error(
                f"Rank {rank}: Error cargando checkpoint: {e}. Iniciando desde cero."
            )
            start_epoch = 0
            best_loss = float('inf')
    else:
    ...
    if rank == 1:
        ...
        if os.path.exists(config.hrnet_model_path):
            ...
            logger.warning(f"Rank {rank}: HRNet no encontrado, solo velocity loss")

    # Dummy input_args
    logger.info(f"Rank {rank}: Generando dummy input_args...")
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        # TRAINING
        iterator = tqdm(train_dataloader, desc=f"Epoch {epoch}") if rank == 0 else train_dataloader
        avg_loss = 0.0
        steps = 0
        ...
        for step, batch in enumerate(iterator):
            ...
            if rank == 0:
                ...
                if isinstance(loss_fn_train, CombinedLossFn):
                    loss_fn_train.set_batch_context(target_heatmaps, timesteps)
        ...
        logger.info(f"Rank {rank}: Ejecutando validación...")
        val_loss = validate(
        ...
        if rank == 0:
            logger.info(
                f"Epoch {epoch} | Train Loss: {global_avg_loss:.6f} | "
                f"Val Loss: {val_loss:.6f} | Mejor histórica: {best_loss:.6f}"
            )
            
            # NUEVO V3: Escribir train_loss Y val_loss en CSV
            with open(csv_file_path, mode="a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([epoch, global_avg_loss, val_loss])
