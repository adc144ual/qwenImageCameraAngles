...
class CombinedLossFn:
    ...
    def __call__(self, outputs: torch.Tensor, combined_target: torch.Tensor) -> torch.Tensor:
        ...
        if (self.vae is not None and
            ...
            try:
                ...
                if self.heatmap_loss_type == "weighted_mse":
                    weights = self.keypoint_weights.to(device).view(1, 27, 1, 1)
        ...
        # )
        current_ratio = velocity_loss_val.item() / (heatmap_loss_val.item() + 1e-8)
        if self.ratio_ema is None:
            self.ratio_ema = current_ratio
        else:
            self.ratio_ema = self.ema_decay * self.ratio_ema + (1 - self.ema_decay) * current_ratio

        # Peso por nivel de ruido: t baja → más confianza en heatmap
        t_norm = self.current_timesteps.float() / 1000.0  # (B,) en [0, 1]
        t_weight = (1.0 - t_norm).mean()                  # scalar en [0, 1]

        heat_scaled = heatmap_loss_val * self.ratio_ema * t_weight

        total_loss = (
            self.velocity_weight * velocity_loss_val +
            self.heatmap_weight  * heat_scaled
        )
        ...
        if self.step_counter < 500:
            if self.step_counter % 10 == 0:
                logger.info(
                    ...
                    f"Ratio EMA: {self.ratio_ema:.1f}, "
                    f"PCK: {self.last_pck:.4f}, "
                    f"Total: {total_loss.item():.6f} "
                    f"(α={self.velocity_weight:.2f}, β={self.heatmap_weight:.2f})"
                    f"t_weight: {t_weight:.3f}, Ratio EMA: {self.ratio_ema:.1f}, "
                )
